...
class Tank:
    ...
    def __str__(self):
        return f"Currant tank position is [{self.x},{self.y}] and it is facing {self.direction}"
...
class Target:
    ...
    def __str__(self):
        return f'Taget position is at [{self.x}, {self.y}]'
...
tank = Tank()
target = Target()
# Making SURE if tank and target is not in the same place
...
while True:
    ...
    if main_action == '1':
        print("Welcome to the tanks game, lets GOOO!")
        while True:
